  ...
  <div class="page">
    ...
    <form v-else ref="editorForm" class="editor-form" @submit.prevent="handleSubmit">
      ...
      <section class="form-section">
        ...
        <div class="form-group full">
          ...
          <input
            ...
            :aria-describedby="errors.title ? 'recruitment-title-error' : undefined"
            placeholder="예: 쇼핑몰 웹 서비스 개발자를 찾습니다"
          />
          <p v-if="errors.title" id="recruitment-title-error" class="form-error" role="alert">
            {{ errors.title }}
          </p>
        </div>
        ...
        <div class="form-group full">
          ...
          <input
            ...
            :aria-describedby="errors.summary ? 'recruitment-summary-error' : undefined"
            placeholder="공고의 핵심을 간단히 소개해주세요"
          />
          <p v-if="errors.summary" id="recruitment-summary-error" class="form-error" role="alert">
            {{ errors.summary }}
          </p>
        </div>
        ...
        <div class="form-grid">
          <div class="form-group">
            <label for="job-category">직종 카테고리 <span class="required">*</span></label>
            <select
              id="job-category"
              v-model="form.jobCategory"
              class="form-control"
              :class="{ invalid: errors.jobCategory }"
              :aria-invalid="Boolean(errors.jobCategory)"
              :aria-describedby="errors.jobCategory ? 'job-category-error' : undefined"
            >
            ...
            <p v-if="errors.jobCategory" id="job-category-error" class="form-error" role="alert">
              {{ errors.jobCategory }}
            </p>
          </div>
          <div class="form-group">
            ...
            <select
              ...
              v-model="form.recruitmentCategory"
              class="form-control"
              :class="{ invalid: errors.recruitmentCategory }"
              :aria-invalid="Boolean(errors.recruitmentCategory)"
              :aria-describedby="
                errors.recruitmentCategory ? 'recruitment-category-error' : undefined
              "
            >
            ...
            <p
              v-if="errors.recruitmentCategory"
              id="recruitment-category-error"
              class="form-error"
              role="alert"
            >
              {{ errors.recruitmentCategory }}
            </p>
          </div>
        </div>
      </section>
      ...
      <section class="form-section">
        <div class="section-heading">
          <h2>상세 내용</h2>
          <p>업무 범위와 기대 결과를 구체적으로 작성해주세요.</p>
        </div>
        <div class="form-group full">
          ...
          <textarea
            ...
            :aria-describedby="errors.content ? 'recruitment-content-error' : undefined"
            placeholder="프로젝트 소개, 주요 업무, 기대 결과를 작성해주세요"
          ></textarea>
          <p v-if="errors.content" id="recruitment-content-error" class="form-error" role="alert">
            {{ errors.content }}
          </p>
        </div>
        <div class="form-group full">
          ...
          <textarea
            ...
            rows="5"
            class="form-control textarea"
            placeholder="필요 경력, 필수 역량, 우대사항을 작성해주세요"
          ></textarea>
        </div>
        <div class="form-group full">
          ...
          <input
            ...
            class="form-control"
            :class="{ invalid: errors.techStacks }"
            :aria-invalid="Boolean(errors.techStacks)"
            :aria-describedby="errors.techStacks ? 'tech-stacks-error' : undefined"
            placeholder="Vue, JavaScript, Figma (쉼표로 구분)"
          />
          <p v-if="errors.techStacks" id="tech-stacks-error" class="form-error" role="alert">
            {{ errors.techStacks }}
          </p>
      ...
      <section class="form-section">
        <div class="section-heading">
          <h2>예산 및 일정</h2>
          <p>일정은 선택 사항이며 시작일보다 종료일이 빠를 수 없습니다.</p>
        </div>
        <div class="form-grid">
          <div class="form-group">
            ...
            <p v-if="errors.budget" id="budget-error" class="form-error" role="alert">
              {{ errors.budget }}
            </p>
          </div>
          <div class="form-group">
        ...
        <div class="form-grid">
          <div class="form-group">
            <label for="recruitment-start">모집 시작일</label
            ><input
              id="recruitment-start"
              v-model="form.recruitmentStartAt"
              type="date"
              class="form-control"
              :class="{ invalid: errors.recruitmentPeriod }"
              :aria-invalid="Boolean(errors.recruitmentPeriod)"
              :aria-describedby="errors.recruitmentPeriod ? 'recruitment-period-error' : undefined"
            />
          </div>
          <div class="form-group">
            <label for="recruitment-end">모집 마감일</label
            ><input
              id="recruitment-end"
              v-model="form.recruitmentEndAt"
              type="date"
              :min="minimumDeadline"
              class="form-control"
              :class="{ invalid: errors.recruitmentPeriod || errors.deadline }"
              :aria-invalid="Boolean(errors.recruitmentPeriod || errors.deadline)"
              :aria-describedby="
                errors.recruitmentPeriod || errors.deadline ? 'recruitment-period-error' : undefined
              "
            />
          </div>
        </div>
        <p
          v-if="errors.deadline || errors.recruitmentPeriod"
          id="recruitment-period-error"
          class="form-error period-error"
          role="alert"
        >
          {{ errors.deadline || errors.recruitmentPeriod }}
        </p>

        <div class="form-grid contract-dates">
          <div class="form-group">
            <label for="contract-start">예상 계약 시작일</label
            ><input
              id="contract-start"
              v-model="form.contractStartAt"
              type="date"
              class="form-control"
              :class="{ invalid: errors.contractPeriod }"
              :aria-invalid="Boolean(errors.contractPeriod)"
              :aria-describedby="errors.contractPeriod ? 'contract-period-error' : undefined"
            />
          </div>
          <div class="form-group">
            <label for="contract-end">예상 계약 종료일</label
            ><input
              id="contract-end"
        ...
        <p
          v-if="errors.contractPeriod"
          id="contract-period-error"
          class="form-error period-error"
          role="alert"
        >
      ...
      <section class="form-section">
        <div class="section-heading">
          <h2>공고 이미지</h2>
          <p>이미지 1개, 최대 10MB까지 등록할 수 있습니다.</p>
        </div>
        ...
        <label class="file-control">
          <input
            id="recruitment-image"
            ref="imageInput"
            type="file"
            accept="image/*"
            :aria-invalid="Boolean(errors.image)"
            :aria-describedby="errors.image ? 'recruitment-image-error' : undefined"
            @change="handleImageChange"
          />
          <span>{{ selectedImageName || (form.imageFileId ? '이미지 변경' : '이미지 선택') }}</span>
        </label>
        <p v-if="errors.image" id="recruitment-image-error" class="form-error" role="alert">
          {{ errors.image }}
        </p>
      </section>

      <div class="form-actions">
        <button type="button" class="cancel-button" :disabled="isSubmitting" @click="handleCancel">
          취소
        </button>
...
async function loadInitialValue() {
  ...
  try {
    ...
    loadError.value = getCompanyApiError(
      error,
...
function validateForm() {
  ...
  if (!form.jobCategory) errors.jobCategory = '직종 카테고리를 선택해주세요.'
  ...
  if (!Number.isFinite(Number(form.budget)) || Number(form.budget) < 0)
    errors.budget = '예산은 0 이상이어야 합니다.'

  const stacks = normalizeTechStacks(form.techStacks)
  if (stacks.some((stack) => stack.length > 50))
    errors.techStacks = '기술 스택 태그는 각각 50자 이하여야 합니다.'
  if (form.recruitmentEndAt && form.recruitmentEndAt < minimumDeadline.value)
    errors.deadline = '모집 마감일은 오늘보다 빠를 수 없습니다.'
  if (
    form.recruitmentStartAt &&
    form.recruitmentEndAt &&
    form.recruitmentStartAt > form.recruitmentEndAt
  )
    errors.recruitmentPeriod = '모집 종료일은 시작일보다 빠를 수 없습니다.'
  if (form.contractStartAt && form.contractEndAt && form.contractStartAt > form.contractEndAt)
    errors.contractPeriod = '계약 종료일은 시작일보다 빠를 수 없습니다.'
  ...
  try {
    ...
    if (isEdit.value) {
      ...
      router.push({
        ...
        params: { recruitmentId: updated?.recruitmentId ?? recruitmentId.value },
      })
      return
    }
    ...
    if (created?.recruitmentId !== null && created?.recruitmentId !== undefined) {
      router.push({
        name: 'CompanyRecruitmentDetail',
        params: { recruitmentId: created.recruitmentId },
      })
    } else {
    ...
    submitError.value = getCompanyApiError(
      error,
      '공고를 저장하지 못했습니다. 입력 내용을 확인하고 다시 시도해주세요.',
    )
  } finally {
    isUploading.value = false
